...
async def send_all_files_to_api(
    ...
    endpoint: API 엔드포인트 URL입니다 (기본값: API_SAVE_ENDPOINT).
    반환값: 성공 시 save.js 응답 dict (job_id 포함), 실패 시 None
    """
    if endpoint is None:
        # 엔드포인트가 지정되지 않으면
        endpoint = API_SAVE_ENDPOINT
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            try:
                ...
                # 응답 데이터를 로그에 출력합니다.
                return resp_data
                # ✅ 응답 dict 그대로 반환 (job_id 포함) — 호출부가 dict.get('job_id') 로 꺼내 씀
            except:
                # 응답 파싱 실패 시
                log(f"[+] 응답 바디: {response.text[:500]}")
                # 응답 텍스트의 처음 500자를 출력합니다.
...
async def custom_scan_endpoint(
    ...
    try:
        ...
        # 모든 파일을 API 서버로 전송
                # 모든 파일을 API 서버로 전송
        api_response = await send_all_files_to_api(final_payload, json_files_to_send)
        ...
        if api_response:
            ...
            log("[+] API 서버 전송 완료")
            # 전송 완료 로그를 출력합니다.
            # ✅ save.js 가 생성한 job_id 를 final_payload 에 넣어 프론트로 전달
            if isinstance(api_response, dict) and api_response.get("job_id"):
                final_payload["job_id"] = api_response["job_id"]
                log(f"[+] job_id 수신: {api_response['job_id']}")
            else:
                log("[!] save.js 응답에 job_id 없음")
        else:
            # API 전송이 실패하면
...
def _sev_badge(sev):
    ...
    # 💡 인라인 스타일로 white-space, display, min-width를 강제 지정합니다.
    return f"""<span style="
...
def build_report_html(data: dict) -> str:
    ...
    if not critical_rows_html:
        ...
        # ── Mismatch 행 (전체, 심각도 내림차순) ──
    mismatch_sorted = sorted(
        mismatch,
        key=lambda x: max(
            severity_rank.get((x.get("grype_severity") or "UNKNOWN").upper(), 0),
            severity_rank.get((x.get("trivy_severity") or "UNKNOWN").upper(), 0),
        ),
        reverse=True
    )
    mismatch_rows_html = ""
    for v in mismatch_sorted:
    ...
    if not mismatch_rows_html:
        ...
        # ── Grype Only 행 (전체, 심각도 내림차순) ──
    grype_only_sorted = sorted(
        grype_only,
        key=lambda x: severity_rank.get((x.get("severity") or "UNKNOWN").upper(), 0),
        reverse=True
    )
    grype_only_rows_html = ""
    for v in grype_only_sorted:
        sev = (v.get("severity") or "UNKNOWN").upper()
        grype_only_rows_html += f"""
        <tr>
          <td class="mono">{v.get('vulnerability_id', '-')}</td>
          <td>{v.get('package_name', '-')}</td>
          <td>{_sev_badge(sev)}</td>
          <td class="mono">{v.get('version', '-') or '-'}</td>
          <td class="mono">{v.get('fix_version', '-') or '-'}</td>
          <td>{v.get('state', '-') or '-'}</td>
        </tr>
        """
    if not grype_only_rows_html:
        grype_only_rows_html = '<tr><td colspan="6" style="text-align:center;color:#6b7280;">Grype Only 항목이 없습니다.</td></tr>'

    # ── Trivy Only 행 (전체, 심각도 내림차순) ──
    trivy_only_sorted = sorted(
        trivy_only,
        key=lambda x: severity_rank.get((x.get("severity") or "UNKNOWN").upper(), 0),
        reverse=True
    )
    trivy_only_rows_html = ""
    for v in trivy_only_sorted:
        sev = (v.get("severity") or "UNKNOWN").upper()
        title_or_desc = (v.get("title") or v.get("description") or "")[:60]
        trivy_only_rows_html += f"""
        <tr>
          <td class="mono">{v.get('vulnerability_id', '-')}</td>
          <td>{v.get('package_name', '-')}</td>
          <td>{_sev_badge(sev)}</td>
          <td class="mono">{v.get('installed_version', '-') or '-'}</td>
          <td class="mono">{v.get('fixed_version', '-') or '-'}</td>
          <td>{title_or_desc}</td>
        </tr>
        """
    if not trivy_only_rows_html:
        trivy_only_rows_html = '<tr><td colspan="6" style="text-align:center;color:#6b7280;">Trivy Only 항목이 없습니다.</td></tr>'
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  h2 {{ 
    ...
    margin-bottom: 14px; color: #1e3a8a; 
    page-break-after: avoid;
  }}
  h3 {{ 
    font-size: 12pt; font-weight: 600; 
    margin: 16px 0 8px; color: #374151; 
    page-break-after: avoid;
  }}
...
<section class="page-break">
  <h2>4. Application Vulnerabilities</h2>
  <table>
    <thead>
      <tr><th>CVE</th><th>Package</th><th>Severity</th><th>Version</th><th>Fix</th><th>Description</th></tr>
    </thead>
    <tbody>
      {critical_rows_html}
    </tbody>
...
<!-- 7. GRYPE ONLY -->
<section>
  <h2>7. Grype Only Detections</h2>
  <p class="text-muted">Grype만 탐지하고 Trivy는 탐지하지 못한 취약점입니다.</p>
  <table>
    <thead>
      <tr><th>CVE</th><th>Package</th><th>Severity</th><th>Version</th><th>Fix</th><th>State</th></tr>
    </thead>
    <tbody>
      {grype_only_rows_html}
    </tbody>
...
<!-- 8. TRIVY ONLY -->
<section>
  <h2>8. Trivy Only Detections</h2>
  <p class="text-muted">Trivy만 탐지하고 Grype는 탐지하지 못한 취약점입니다.</p>
  <table>
    <thead>
      <tr><th>CVE</th><th>Package</th><th>Severity</th><th>Version</th><th>Fix</th><th>Description</th></tr>
    </thead>
...
async def generate_report_endpoint(request: Request):
    ...
    try:
        ...
        def _make_pdf():
            # 동기 WeasyPrint 호출을 별도 함수로 분리합니다.
            from weasyprint import HTML
            # weasyprint를 lazy 임포트합니다. (pip install weasyprint)
            HTML(string=html_string).write_pdf(pdf_local_path)

        await loop.run_in_executor(None, _make_pdf)
        # 블로킹 호출을 스레드풀에서 실행합니다.
        log(f"[+] PDF 생성 완료: {pdf_local_path}")
...
# ──────────────────────────────────────────────
# 상세 보고서 HTML 빌드 (정제 데이터 전체 수록)
# ──────────────────────────────────────────────
def build_detail_report_html(data: dict) -> str:
    """
    fetch_report_data 가 가져온 정제 데이터(OS/App/Secrets/grype_only/trivy_only/mismatch)
    를 전부 표 형태로 수록한 상세 보고서 HTML 을 만듭니다.
    Grype/Trivy 원시 JSON 은 제외합니다.
    """
    scan_job    = data.get("scan_job", {}) or {}
    scan_report = data.get("scan_report", {}) or {}
    os_vulns    = data.get("os_vulns", []) or []
    ...
    # ── 헬퍼: 심각도 내림차순 정렬 ─────────────────────────────
    def _sort_by_sev(rows, sev_key="severity"):
        return sorted(
            rows,
            key=lambda x: severity_rank.get((x.get(sev_key) or "UNKNOWN").upper(), 0),
            reverse=True
        )

    # ── OS 취약점 전체 ─────────────────────────────────────────
    os_rows_html = ""
    for v in _sort_by_sev(os_vulns):
        sev = (v.get("severity") or "UNKNOWN").upper()
        os_rows_html += f"""
        <tr>
          <td class="mono">{v.get('vulnerability_id', '-')}</td>
          ...
          <td>{v.get('state', '-') or '-'}</td>
          <td class="mono">{v.get('fix_version', '-') or '-'}</td>
          <td>{v.get('package_type', '-') or '-'}</td>
        </tr>
        """
    if not os_rows_html:
        os_rows_html = '<tr><td colspan="7" style="text-align:center;color:#6b7280;">데이터 없음</td></tr>'

    # ── App 취약점 전체 ────────────────────────────────────────
    app_rows_html = ""
    for v in _sort_by_sev(app_vulns):
        sev = (v.get("severity") or "UNKNOWN").upper()
        desc = (v.get("title") or v.get("description") or "")[:70]
        app_rows_html += f"""
        <tr>
        <td class="nowrap">{v.get('vulnerability_id', '-')}</td>
        <td>{v.get('package_name', '-')}</td>
        <td class="mono">{v.get('installed_version', '-') or '-'}</td>
        <td>{_sev_badge(sev)}</td>
        <td class="mono">{v.get('fixed_version', '-') or '-'}</td>
        <td style="word-break: break-all; min-width: 150px; max-width: 250px;">{v.get('target', '-') or '-'}</td>
        <td>{desc}</td>
        </tr>
    """
    if not app_rows_html:
        app_rows_html = '<tr><td colspan="7" style="text-align:center;color:#6b7280;">데이터 없음</td></tr>'

    # ── 시크릿 전체 ────────────────────────────────────────────
    sec_rows_html = ""
    for s in secrets:
        sev = (s.get("severity") or "UNKNOWN").upper()
        match_text = (s.get("match_text") or "")[:80]
        ld = s.get("layer_digest") or ""
        sec_rows_html += f"""
        <tr>
          <td>{s.get('rule_id', '-')}</td>
          ...
          <td>{_sev_badge(sev)}</td>
          <td class="mono">{match_text}</td>
          <td class="mono">{(ld[:24] + '...') if ld else '-'}</td>
        </tr>
        """
    if not sec_rows_html:
        sec_rows_html = '<tr><td colspan="5" style="text-align:center;color:#6b7280;">데이터 없음</td></tr>'

    # ── Grype Only 전체 ────────────────────────────────────────
    grype_only_rows_html = ""
    for v in _sort_by_sev(grype_only):
        sev = (v.get("severity") or "UNKNOWN").upper()
        grype_only_rows_html += f"""
        <tr>
          <td class="mono">{v.get('vulnerability_id', '-')}</td>
        ...
        """
    if not grype_only_rows_html:
        grype_only_rows_html = '<tr><td colspan="6" style="text-align:center;color:#6b7280;">데이터 없음</td></tr>'

    # ── Trivy Only 전체 ────────────────────────────────────────
    trivy_only_rows_html = ""
    for v in _sort_by_sev(trivy_only):
        sev = (v.get("severity") or "UNKNOWN").upper()
        desc = (v.get("title") or v.get("description") or "")[:80]
        trivy_only_rows_html += f"""
        <tr>
          <td class="mono">{v.get('vulnerability_id', '-')}</td>
          <td>{v.get('package_name', '-')}</td>
        ...
        """
    if not trivy_only_rows_html:
        trivy_only_rows_html = '<tr><td colspan="6" style="text-align:center;color:#6b7280;">데이터 없음</td></tr>'
    ...
    for v in mismatch_sorted:
        ...
        <tr>
          ...
          <td>{_sev_badge(t_sev)}</td>
          <td>{_sev_badge(rec)}</td>
          <td class="mono">{v.get('grype_fix_version', '-') or v.get('trivy_fixed_version', '-') or '-'}</td>
        </tr>
        """
    if not mismatch_rows_html:
        mismatch_rows_html = '<tr><td colspan="7" style="text-align:center;color:#6b7280;">데이터 없음</td></tr>'
...
<head>
<meta charset="UTF-8">
<title>Container Security Detail Report</title>
<style>
  @page {{ size: A4; margin: 18mm 12mm;
    @bottom-center {{ content: "Page " counter(page) " / " counter(pages); font-size: 9pt; color: #888; }}
  }}
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{ font-family: 'Noto Sans KR', 'Malgun Gothic', sans-serif; font-size: 9pt; color: #1a1a1a; line-height: 1.5; }}

  .cover {{ page-break-after: always; height: 257mm; background: #0f766e; color: white; padding: 50px 40px; margin: -18mm -12mm 0 -12mm; position: relative; }}
  .cover-tag {{ font-size: 11pt; letter-spacing: 4px; text-transform: uppercase; opacity: 0.7; margin-top: 60px; }}
  .cover-title {{ font-size: 36pt; font-weight: 800; line-height: 1.2; margin-top: 20px; }}
  .cover-subtitle {{ font-size: 14pt; opacity: 0.85; margin-top: 12px; }}
  .cover-meta {{ position: absolute; bottom: 60px; left: 40px; right: 40px; border-top: 1px solid rgba(255,255,255,0.3); padding-top: 24px; font-size: 11pt; }}
  ...
  h2 {{
    ...
    border-left: 4px solid #0f766e; padding-left: 12px;
    margin-bottom: 12px; color: #0f766e;
    page-break-after: avoid;
  }}
  .section-meta {{ font-size: 9pt; color: #6b7280; margin-bottom: 8px; }}

  .stats-row {{ display: flex; gap: 8px; margin-bottom: 16px; }}
  .stat-card {{ flex: 1; border: 1px solid #e5e7eb; border-radius: 6px; padding: 10px; text-align: center; background: #fafafa; }}
  .stat-num {{ font-size: 18pt; font-weight: 800; color: #0f766e; }}
  .stat-label {{ font-size: 8pt; text-transform: uppercase; letter-spacing: 0.5px; color: #6b7280; margin-top: 4px; }}

  table {{ width: 100%; border-collapse: collapse; font-size: 8.5pt; margin-top: 6px; page-break-inside: auto; }}
  thead {{ display: table-header-group; }}
  tbody {{ display: table-row-group; }}
  tr {{ page-break-inside: avoid; }}
  th {{ background: #0f766e; color: white; padding: 6px 8px; text-align: left; font-weight: 600; font-size: 8pt; text-transform: uppercase; letter-spacing: 0.3px; }}
  td {{ padding: 5px 8px; border-bottom: 1px solid #e5e7eb; vertical-align: top; word-wrap: break-word;word-break: break-all; white-space: normal; }}
  tr:nth-child(even) td {{ background: #fafbfc; }}
  td.mono {{ font-family: Consolas, monospace; font-size: 8pt; }}
td.mono-wrap {{ font-family: Consolas, monospace; font-size: 8pt; word-break: break-all; }}
td.nowrap {{ white-space: nowrap; font-family: Consolas, monospace; font-size: 8pt; }}

/* 컬럼 폭 힌트 */
table.compact th:nth-child(1),
table.compact td:nth-child(1) {{ width: 18%; }}


  .sev {{ display: inline-block; padding: 2px 6px; border-radius: 3px; font-size: 7.5pt; font-weight: 700; text-transform: uppercase; color: white; }}
  .sev-critical {{ background: #dc2626; }}
  .sev-high {{ background: #ea580c; }}
  .sev-medium {{ background: #ca8a04; }}
  .sev-low {{ background: #16a34a; }}
...
<!-- COVER -->
<div class="cover">
  <div class="cover-tag">Detail Vulnerability Data</div>
  <div class="cover-title">상세 데이터 보고서</div>
  <div class="cover-subtitle">{image_name}</div>
  <div class="cover-meta">
    <div><strong>Scan ID</strong> {scan_id}</div>
    <div><strong>Image</strong> {image_name}</div>
    <div><strong>Scanners</strong> Grype + Trivy</div>
    <div><strong>Generated</strong> {created_at}</div>
    <div><strong>Type</strong> 가공 데이터 전체 (RAW 제외)</div>
  </div>
</div>
...
<section>
  <h2>0. Overview</h2>
  <div class="stats-row">
    <div class="stat-card"><div class="stat-num">{total_count}</div><div class="stat-label">Total Unique</div></div>
    <div class="stat-card"><div class="stat-num">{common_count}</div><div class="stat-label">Common</div></div>
    <div class="stat-card"><div class="stat-num">{grype_only_cnt}</div><div class="stat-label">Grype Only</div></div>
    <div class="stat-card"><div class="stat-num">{trivy_only_cnt}</div><div class="stat-label">Trivy Only</div></div>
    <div class="stat-card"><div class="stat-num">{mismatch_cnt}</div><div class="stat-label">Mismatch</div></div>
  </div>
</section>

<!-- 1. OS Filtered -->
<section>
  <h2>1. OS Filtered (Grype OS Vulnerabilities)</h2>
  <div class="section-meta">총 {len(os_vulns)} 건</div>
  <table>
    <thead>
      <tr><th>CVE</th><th>Package</th><th>Version</th><th>Severity</th><th>State</th><th>Fix Version</th><th>Type</th></tr>
    </thead>
    <tbody>
      {os_rows_html}
    </tbody>
...
<!-- 2. Application Filtered -->
<section>
  <h2>2. Application Filtered (Trivy App Vulnerabilities)</h2>
  <div class="section-meta">총 {len(app_vulns)} 건</div>
  <table>
    <thead>
      <tr><th>CVE</th><th>Package</th><th>Installed</th><th>Severity</th><th>Fixed</th><th>Target</th><th>Description</th></tr>
    </thead>
    <tbody>
      {app_rows_html}
    </tbody>
...
<!-- 3. Security Filtered (Secrets) -->
<section>
  <h2>3. Security Filtered (Secrets)</h2>
  <div class="section-meta">총 {len(secrets)} 건</div>
  <table>
    <thead>
      <tr><th>Rule ID</th><th>Category</th><th>Severity</th><th>Match Text</th><th>Layer Digest</th></tr>
    </thead>
    <tbody>
      {sec_rows_html}
    </tbody>
...
<!-- 4. Grype Only -->
<section>
  <h2>4. Grype Only Detections</h2>
  <div class="section-meta">Grype 만 탐지한 취약점 · 총 {len(grype_only)} 건</div>
  <table>
    <thead>
      <tr><th>CVE</th><th>Package</th><th>Version</th><th>Severity</th><th>State</th><th>Fix Version</th></tr>
    </thead>
    <tbody>
      {grype_only_rows_html}
    </tbody>
...
<!-- 5. Trivy Only -->
<section>
  <h2>5. Trivy Only Detections</h2>
  <div class="section-meta">Trivy 만 탐지한 취약점 · 총 {len(trivy_only)} 건</div>
  <table>
    <thead>
      <tr><th>CVE</th><th>Package</th><th>Installed</th><th>Severity</th><th>Fixed</th><th>Description</th></tr>
    </thead>
    <tbody>
      {trivy_only_rows_html}
    </tbody>
...
<!-- 6. Mismatch -->
<section>
  <h2>6. Severity Mismatch</h2>
  <div class="section-meta">두 도구가 동일 CVE 를 다른 심각도로 평가한 경우 · 총 {len(mismatch)} 건</div>
  <table>
    <thead>
      <tr><th>CVE</th><th>Package</th><th>Version</th><th>Grype</th><th>Trivy</th><th>Recommended</th><th>Fix</th></tr>
    </thead>
    <tbody>
      {mismatch_rows_html}
    </tbody>
...
<div style="margin-top:40px;text-align:center;font-size:8pt;color:#9ca3af;border-top:1px solid #e5e7eb;padding-top:14px;">
  본 보고서는 자동 생성되었습니다. · Scan ID: {scan_id} · {created_at} · 가공 데이터 전체 (RAW 제외)
</div>
...
# ──────────────────────────────────────────────
# 상세 PDF 생성 + Storage 업로드 엔드포인트
# RAW Grype/Trivy 데이터 제외, 가공된 정제 데이터 전체 수록
# ──────────────────────────────────────────────
@app.post("/report/generate-detail")
async def generate_detail_report_endpoint(request: Request):
    """
    요청 body (JSON):
      { "scan_job_id": "<UUID>" }

    동작 순서 (기존 /report/generate 와 동일하지만 build_detail_report_html 사용):
      1. DB 조회 (fetch_report_data)
      2. 상세 HTML 빌드
      3. WeasyPrint 로 PDF 생성
      4. Supabase Storage 에 업로드 (경로: <scan_job_id>/report_detail.pdf)

    응답:
      { "status": "success", "storage_path": "<scan_job_id>/report_detail.pdf" }
    """
    try:
        body = await request.json()
        scan_job_id = body.get("scan_job_id")
        ...
        log(f"[*] 상세 PDF 보고서 생성 시작: scan_job_id={scan_job_id}")
        ...
        log(
            ...
            f"App={len(report_data['app_vulns'])} / "
            f"Secrets={len(report_data['secrets'])} / "
            f"GrypeOnly={len(report_data['grype_only'])} / "
            f"TrivyOnly={len(report_data['trivy_only'])} / "
            f"Mismatch={len(report_data['mismatch'])}"
        )

        # ── 2단계: 상세 HTML 빌드 ─────────────────────────────────
        html_string = build_detail_report_html(report_data)
        log(f"[+] 상세 HTML 빌드 완료 ({len(html_string)} bytes)")

        # ── 3단계: PDF 생성 ───────────────────────────────────────
        pdf_local_path = work_path(f"report_detail_{scan_job_id}.pdf")
        loop = asyncio.get_event_loop()
        ...
        await loop.run_in_executor(None, _make_pdf)
        log(f"[+] 상세 PDF 생성 완료: {pdf_local_path}")
        ...
        storage_path = f"{scan_job_id}/report_detail.pdf"
        ...
        try:
            await loop.run_in_executor(None, _upload)
            log(f"[+] Storage 업로드 완료 (상세): {SUPABASE_BUCKET}/{storage_path}")
        except Exception as upload_err:
            log(f"[!] Storage 업로드 실패 (상세): {upload_err}")
            return {"status": "error", "detail": f"upload failed: {upload_err}"}
        ...
        log(f"[!] 상세 PDF 생성 중 예외 발생: {type(e).__name__}: {str(e)[:500]}")
        return {"status": "error", "detail": str(e)}
