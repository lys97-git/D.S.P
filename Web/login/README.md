...
`c:\login`에서 바로 실행 가능한 심플 로그인 웹페이지입니다.
...
`user_profiles`를 `profiles`로 통합하려면 Supabase `SQL Editor`에서
[supabase_migrate_user_profiles_to_profiles.sql](C:/login/supabase_migrate_user_profiles_to_profiles.sql)
전체를 실행하세요.

처리 항목:
- `user_profiles` 데이터 -> `profiles` 이관
- `user_profiles` 참조 FK -> `profiles(id)` 이전
- `auth.users` 연동 트리거 재구성
- RLS 정책 / ID,PW 찾기 RPC 함수 재구성
- `user_profiles` 삭제
