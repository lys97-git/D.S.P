...
# [콜백 1-1: 버튼 클릭 직후 실시간 로그 업데이트 채널]
@callback(
...
def trigger_scan_log(n_clicks, filename, current_logs):
...
# [콜백 1-2: 준비 완료 상태를 이어받아 백엔드 무거운 연산 처리 수행]
@callback(
...
def perform_scan(status, contents, filename, current_logs):
    ...
    try:
        ...
        if response.status_code == 200:
            logs.insert(0, {"time": datetime.datetime.now().strftime("%H:%M:%S"), "content": "분석 완료 및 데이터 수신 성공"})
            return response.json(), logs, "idle", False, False
        else:
...
def update_pie_chart(data, tool_filter):
    ...
    if tool_filter != "ALL":
        ...
        if df.empty:
            return get_empty_state(f"{tool_filter.upper()} 결과 없음")
    ...
    # [핵심 수정] 원형 차트가 전체 영역 크기를 기준으로 정중앙에 그리도록 도메인 강제 지정
    fig.update_traces(
    ...
    # [핵심 수정] 범례를 하단(Horizontal) 배치하고 대칭 마진을 주어 수치 왜곡 원천 차단
    fig.update_layout(
        showlegend=True,
...
def update_table(data, tool_filter, class_filter, selected_severity, sort_clicks, sort_state):
    ...
    if class_filter != "ALL":
        def classify_row(row):
            r_class = str(row.get('result_class', '')).lower()
            r_type = str(row.get('result_type', '')).lower()
            combined_info = f"{r_class} {r_type}"
            if 'secret' in combined_info: return 'SECRET'
            if any(kw in combined_info for kw in ['os', 'debian', 'ubuntu', 'alpine', 'centos', 'redhat', 'amzn', 'apk', 'dpkg', 'rpm']):
                return 'OS'
            return 'APP' 
