...
async def custom_scan_endpoint(
    ...
    try:
        ...
        # ── v3 대시보드 호환 어댑터 ──────────────────────────────────────────
        # front_end_v3.py는 scan_result["summary"], scan_result["vulnerabilities"]를 직접 사용합니다.
        def _cap(sev):
            return {
                "CRITICAL": "Critical", "HIGH": "High",
                "MEDIUM": "Medium", "LOW": "Low",
            }.get(str(sev or "").upper(), "Unknown")
        ...
        for v in filtered_data.get("grype_vulnerabilities", []):
            ...
            dash_vulns.append({
                ...
                "pkg_name":          v.get("package_name") or "",
                "pkg_path":          v.get("install_path") or "-",
                "installed_version": v.get("version") or "-",
                "fixed_version":     v.get("fix_version") or "N/A",
        ...
        for v in filtered_data.get("trivy_vulnerabilities", []):
            ...
            dash_vulns.append({
                "vulnerability_id": vid,
                "pkg_name":          v.get("package_name") or "",
                "pkg_path":          v.get("package_path") or "-",
                "installed_version": v.get("installed_version") or "-",
                "fixed_version":     v.get("fixed_version") or "N/A",
                "severity":          _cap(v.get("severity")),
                "description":       v.get("description") or "",
            })

        dash_summary = {"Critical": 0, "High": 0, "Medium": 0, "Low": 0}
