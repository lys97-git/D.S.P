...
from flask import send_from_directory, redirect, Response, make_response
...
_LOGIN_DIR = os.path.join(_BASE_DIR, 'login')
_DYNAMIC_DIR = os.path.join(_BASE_DIR, 'dynamic')
_LOGIN_CONFIG_PATH = os.path.join(_LOGIN_DIR, 'config.js')
_SCANNER_BASE_URL = os.environ.get("PYTHON_SCANNER_URL", "http://localhost:8000").rstrip("/")
_DYNAMIC_API_BASE = os.environ.get("DYNAMIC_API_BASE", f"{_SCANNER_BASE_URL}/dynamic-api")
_DYNAMIC_ASSET_VERSION = str(
    max(
        int(os.path.getmtime(os.path.join(_DYNAMIC_DIR, name)))
        for name in ("dynamic_page.html", "dynamic_page.css", "dynamic_page.js")
        if os.path.exists(os.path.join(_DYNAMIC_DIR, name))
    )
)


def _read_supabase_client_config():
    config = {"supabaseUrl": "", "supabaseAnonKey": ""}
    try:
        with open(_LOGIN_CONFIG_PATH, "r", encoding="utf-8") as config_file:
            raw = config_file.read()
    except OSError:
        return config

    for source_key, target_key in (("SUPABASE_URL", "supabaseUrl"), ("SUPABASE_ANON_KEY", "supabaseAnonKey")):
        match = re.search(rf'{source_key}\s*=\s*"([^"]+)"', raw)
        if match:
            config[target_key] = match.group(1)
    return config
...
@server.route('/dynamic-app')
def _serve_dynamic_redirect():
    return redirect('/dynamic-app/', code=302)


@server.route('/dynamic-app/')
def _serve_dynamic_index():
    response = make_response(send_from_directory(_DYNAMIC_DIR, 'dynamic_page.html'))
...
@server.route('/dynamic-app/config.js')
def _serve_dynamic_config():
    payload = _read_supabase_client_config()
    payload["apiBase"] = _DYNAMIC_API_BASE
    payload["vmHost"] = os.environ.get("DYNAMIC_VM_HOST", "192.168.25.132")
    payload["vmUser"] = os.environ.get("DYNAMIC_VM_USER", "pro")
    payload["vmPort"] = os.environ.get("DYNAMIC_VM_PORT", "22")
    body = f"window.DSP_DYNAMIC_CONFIG = {json.dumps(payload)};"
...
@server.route('/dynamic-app/<path:filename>')
def _serve_dynamic_assets(filename):
    response = make_response(send_from_directory(_DYNAMIC_DIR, filename))
    response.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
    response.headers["Pragma"] = "no-cache"
    response.headers["Expires"] = "0"
    return response
                    ...
                    children=[
                        dmc.Stack(gap="md", children=[
                                            ...
                                            dmc.Stack(
                                                ...
                                                children=[
                                                    dmc.Group(
                                                        gap="sm",
                                                        children=[
                                                            DashIconify(icon="tabler:activity-heartbeat", width=26, color=THEME_TEXT_ACCENT),
                                                            dmc.Title("동적 분석", order=3, style={"color": THEME_TEXT_ACCENT}),
                                                        ],
                                                    ),
                                                    dmc.Text(
                                                        "기존 dynamic 기능을 DSP 대시보드 안으로 연결했습니다. 현재 세션을 그대로 사용하면서 "
                                                        "이미지 적재, 컨테이너 실행 제어, 실시간 이벤트 감시를 한 화면에서 이어서 볼 수 있습니다.",
                                                        size="sm",
                                                        c=THEME_TEXT_MAIN,
                                                    ),
                                                ],
                                            ),
                                            dmc.Badge("DSP 연동", color="teal", variant="light", size="lg"),
                                        ],
                                    )
                                ],
                            ),
                            dmc.Paper(
                                ...
                                children=[
                                    html.Iframe(
                                        src=f"/dynamic-app/?embedded=1&v={_DYNAMIC_ASSET_VERSION}",
                        ...
                        dmc.Paper(
                            ...
                            style={"backgroundColor": THEME_BG_PAPER, "borderColor": THEME_BORDER, "display": "none"},
                            children=[
...
def switch_pages(btn_image, btn_dynamic):
    ...
    if trigger == "nav-dynamic-btn":
        # 동적분석 활성화, 이미지 숨김
        return {"display": "none"}, {"display": "block"}, "subtle", "gray", "light", "teal", {"display": "none"}
    else:
        # 이미지분석 활성화, 동적 숨김
        return {"display": "block"}, {"display": "none"}, "light", "teal", "subtle", "gray", {"display": "inline-flex"}
...
def perform_scan(status, contents, filename, current_logs, token):
    ...
    try:
        ...
        try:
            with tarfile.open(fileobj=io.BytesIO(decoded), mode="r:*"):
                pass
        except tarfile.TarError:
            logs.insert(0, {
                "time": datetime.datetime.now().strftime("%H:%M:%S"),
                "content": f"유효한 Docker image tar가 아닙니다: {filename} ({size_mb:.2f} MB)"
            })
        ...
        response = requests.post(
            ...
            data={'user_id': 'admin', 'auth_token': token or ''},
            headers=headers,
            timeout=1800
        )
        ...
        if response.status_code == 200:
            ...
            final_scan_job = final_data.get("scan_job", {}) or scan_data.get("scan_job", {})
            vuln_cnt = len(final_data.get("vulnerabilities", []))
            if final_scan_job.get("status") == "failed":
                logs.insert(0, {
                    "time": datetime.datetime.now().strftime("%H:%M:%S"),
                    "content": (
                        f"스캔 실패 - Trivy: {final_scan_job.get('trivy_status', '-')}, "
                        f"Grype: {final_scan_job.get('grype_status', '-')}, "
                        f"결과: {vuln_cnt}건"
                    )
                })
            ...
            detail = ""
            try:
                detail = (response.json().get("detail") or response.text or "")[:160]
            except Exception:
                detail = (response.text or "")[:160]
            logs.insert(0, {"time": datetime.datetime.now().strftime("%H:%M:%S"), "content": f"서버 에러 발생: {response.status_code} {detail}"})
            return no_update, logs, "idle", False, False
