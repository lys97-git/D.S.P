...
# API 서버 주소 설정
API_SERVER_URL = os.environ.get("API_SERVER_URL", "http://127.0.0.1:3000").strip().rstrip("/")
...
async def send_to_api_server(payload: dict, endpoint: str = None, auth_token: str = "") -> dict:
    """
    ...
    try:
        ...
        response = await loop.run_in_executor(
            None,
            lambda: requests.post(endpoint, json=payload, headers=auth_headers, timeout=API_TIMEOUT)
        )
        ...
        # 타임아웃 로그를 출력합니다.
        return {"ok": False, "job_id": None, "response": None, "error": "timeout", "status_code": None}
    except requests.exceptions.ConnectionError:
        # 연결 실패 예외 발생 시
        log(f"[!] API 서버 연결 실패: {endpoint}")
        # 연결 실패 로그를 출력합니다.
        return {"ok": False, "job_id": None, "response": None, "error": "connection_error", "status_code": None}
    except Exception as e:
        # 기타 예외 발생 시
        log(f"[!] API 전송 중 예외 발생: {type(e).__name__}: {str(e)[:300]}")
        # 예외 정보를 로그에 출력합니다.
        return {"ok": False, "job_id": None, "response": None, "error": str(e)[:300], "status_code": None}
...
async def custom_scan_endpoint(
    ...
    form_auth_token = (auth_token or "").strip()
    auth_header = request.headers.get("authorization") or ""
    header_auth_token = auth_header.split(" ", 1)[1].strip() if auth_header.lower().startswith("bearer ") else ""
    effective_auth_token = header_auth_token or form_auth_token
    if effective_auth_token:
        log(f"[*] Authorization token received (Bearer ...{effective_auth_token[-8:]}) -> relay to Node /save")
    else:
        log("[*] Authorization token missing -> Node /save may reject this scan with 401")
    ...
    try:
        ...
        try:
            ...
            # 에러 전송 시작 로그를 출력합니다.
            await send_to_api_server(error_payload, auth_token=effective_auth_token)
...
def build_detail_report_html(data: dict) -> str:
    """Build a detailed HTML report for the detail PDF export."""
    ...
    def sev(value):
        return _sev_badge((value or "UNKNOWN").upper())

    def sort_by_severity(rows, key="severity"):
        return sorted(
            rows,
            key=lambda item: severity_rank.get((item.get(key) or "UNKNOWN").upper(), 0),
            reverse=True,
        )

    def build_rows(rows, columns, empty_colspan):
        if not rows:
            return f'<tr><td colspan="{empty_colspan}" style="text-align:center;color:#6b7280;">데이터 없음</td></tr>'

        rendered = []
        for row in rows:
            rendered.append("<tr>")
            for renderer in columns:
                rendered.append(renderer(row))
            rendered.append("</tr>")
    ...
    created_at_raw = scan_job.get("created_at") or ""
    created_at = created_at_raw[:16].replace("T", " ") if created_at_raw else "-"
    total_count = scan_report.get("total_count") or (len(os_vulns) + len(app_vulns))
    common_count = scan_report.get("common_count", 0)
    ...
    os_rows_html = build_rows(
        sort_by_severity(os_vulns),
        [
            lambda row: f'<td class="mono">{esc(row.get("vulnerability_id"))}</td>',
            lambda row: f"<td>{esc(row.get('package_name'))}</td>",
            lambda row: f'<td class="mono">{esc(row.get("version") or row.get("installed_version"))}</td>',
            lambda row: f"<td>{sev(row.get('severity'))}</td>",
            lambda row: f"<td>{esc(row.get('state'))}</td>",
            lambda row: f'<td class="mono">{esc(row.get("fix_version") or row.get("fixed_version"))}</td>',
            lambda row: f"<td>{esc(row.get('package_type') or row.get('result_type'))}</td>",
        ],
        7,
    )

    app_rows_html = build_rows(
        sort_by_severity(app_vulns),
        [
            lambda row: f'<td class="mono-wrap">{esc(row.get("vulnerability_id"))}</td>',
            lambda row: f"<td>{esc(row.get('package_name'))}</td>",
            lambda row: f'<td class="mono">{esc(row.get("installed_version") or row.get("version"))}</td>',
            lambda row: f"<td>{sev(row.get('severity'))}</td>",
            lambda row: f'<td class="mono">{esc(row.get("fixed_version") or row.get("fix_version"))}</td>',
            lambda row: f"<td>{esc(row.get('target'))}</td>",
            lambda row: f"<td>{esc((row.get('title') or row.get('description') or '-')[:120])}</td>",
        ],
        7,
    )

    secret_rows_html = build_rows(
        secrets,
        [
            lambda row: f"<td>{esc(row.get('rule_id'))}</td>",
            lambda row: f"<td>{esc(row.get('category'))}</td>",
            lambda row: f"<td>{sev(row.get('severity'))}</td>",
            lambda row: f'<td class="mono-wrap">{esc((row.get("match_text") or "-")[:120])}</td>',
            lambda row: f'<td class="mono">{esc(((row.get("layer_digest") or "-")[:24] + "...") if row.get("layer_digest") and len(row.get("layer_digest")) > 24 else row.get("layer_digest"))}</td>',
        ],
        5,
    )

    grype_only_rows_html = build_rows(
        sort_by_severity(grype_only),
        [
            lambda row: f'<td class="mono">{esc(row.get("vulnerability_id"))}</td>',
            lambda row: f"<td>{esc(row.get('package_name'))}</td>",
            lambda row: f'<td class="mono">{esc(row.get("version") or row.get("installed_version"))}</td>',
    ...
    trivy_only_rows_html = build_rows(
        sort_by_severity(trivy_only),
        [
            lambda row: f'<td class="mono">{esc(row.get("vulnerability_id"))}</td>',
            lambda row: f"<td>{esc(row.get('package_name'))}</td>",
            lambda row: f'<td class="mono">{esc(row.get("installed_version") or row.get("version"))}</td>',
    ...
    mismatch_rows_html = build_rows(
        sorted(
            mismatch,
            key=lambda item: max(
                severity_rank.get((item.get("grype_severity") or "UNKNOWN").upper(), 0),
                severity_rank.get((item.get("trivy_severity") or "UNKNOWN").upper(), 0),
            ),
            reverse=True,
        ),
        [
            ...
            lambda row: f"<td>{esc(row.get('package_name'))}</td>",
            lambda row: f'<td class="mono">{esc(row.get("version") or row.get("installed_version"))}</td>',
            lambda row: f"<td>{sev(row.get('grype_severity'))}</td>",
            lambda row: f"<td>{sev(row.get('trivy_severity'))}</td>",
            lambda row: f"<td>{sev((row.get('grype_severity') if severity_rank.get((row.get('grype_severity') or 'UNKNOWN').upper(), 0) >= severity_rank.get((row.get('trivy_severity') or 'UNKNOWN').upper(), 0) else row.get('trivy_severity')))}</td>",
            lambda row: f'<td class="mono">{esc(row.get("grype_fix_version") or row.get("trivy_fixed_version") or row.get("fix_version") or row.get("fixed_version"))}</td>',
        ],
        7,
    )
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  h2 {{ font-size: 15pt; font-weight: 700; border-left: 4px solid #0f766e; padding-left: 12px; margin-bottom: 12px; color: #0f766e; page-break-after: avoid; }}
...
<div class="cover">
  ...
  <div class="cover-subtitle">{esc(image_name)}</div>
  <div class="cover-meta">
    <div><strong>Scan ID</strong> {esc(scan_id)}</div>
    <div><strong>Image</strong> {esc(image_name)}</div>
    <div><strong>Scanners</strong> Grype + Trivy</div>
    <div><strong>Generated</strong> {esc(created_at)}</div>
...
<section>
  <h2>0. Overview</h2>
  <div class="stats-row">
    <div class="stat-card"><div class="stat-num">{total_count}</div><div class="stat-label">Total</div></div>
    <div class="stat-card"><div class="stat-num">{common_count}</div><div class="stat-label">Common</div></div>
    <div class="stat-card"><div class="stat-num">{grype_only_count}</div><div class="stat-label">Grype Only</div></div>
    <div class="stat-card"><div class="stat-num">{trivy_only_count}</div><div class="stat-label">Trivy Only</div></div>
    <div class="stat-card"><div class="stat-num">{mismatch_count}</div><div class="stat-label">Mismatch</div></div>
  </div>
</section>
...
@app.post("/report/generate_detail")
async def generate_detail_report_endpoint(request: Request):
    """
    Request body:
      { "scan_job_id": "<UUID>" }

    Generates the detailed PDF report and uploads it to Supabase Storage as
    <scan_job_id>/report_detail.pdf.
    """
    try:
        body = await request.json()
        scan_job_id = body.get("scan_job_id")
        ...
        pdf_engine = await loop.run_in_executor(None, _make_pdf)
        log(f"[+] 상세 PDF 생성 완료 ({pdf_engine}): {pdf_local_path}")
        ...
        with open(pdf_local_path, "rb") as handle:
